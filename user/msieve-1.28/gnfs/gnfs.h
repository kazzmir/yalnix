...
typedef struct {
	...
	uint32 rfb_lp_cutoff_bits;   /* rational size that triggers LP code */
	uint32 afb_lp_cutoff_bits;   /* algebraic size that triggers LP code */
...
/* print the size and root properties of one polynomial */
...
/* the minimum number of excess columns in the final
   ...
   chosen also has a healthy fudge factor */
...
/* given a relation, find and list all of the rational
   ideals whose prime exceeds filtmin_r and all of the
   algebraic ideals whose prime exceeds filtmin_a. If
   both of these bounds are zero then all ideals are listed */
...
/* Assuming a group of relations has been grouped together
   ...
   occurs an odd number of times in the relation */
