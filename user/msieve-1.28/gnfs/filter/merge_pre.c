...
void nfs_merge_2way(msieve_obj *obj, filter_t *filter,
	...
	ideal_map = (ideal_map_t *)calloc((size_t)num_ideals, 
					sizeof(ideal_map_t));
	...
	reverse_array = (ideal_relation_t *)malloc(num_reverse_alloc *
	...
	for (i = 0; i < num_relations; i++) {
		...
		for (j = 0; j < curr_relation->ideal_count; j++) {
			...
			if (num_reverse == num_reverse_alloc) {
				num_reverse_alloc *= 2;
				reverse_array = (ideal_relation_t *)realloc(
						reverse_array,
						num_reverse_alloc *
						sizeof(ideal_relation_t));
			}
	...
	relset_array = (relation_set_t *)malloc(num_relset_alloc *
	...
	for (i = 0; i < num_relations; 
		...
		/* clique is complete; allocate a relation set for it */

		if (num_tmp_ideal >= MAX_2WAY_IDEALS || 
		    num_tmp_relation >= MAX_2WAY_RELATIONS) {
			printf("error: 2-way merge resources exceeded\n");
			exit(-1);
		}
		if (num_relset == num_relset_alloc) {
			num_relset_alloc *= 2;
			relset_array = (relation_set_t *)realloc(relset_array,
							num_relset_alloc *
							sizeof(relation_set_t));
		}
		...
		/* sort the relation numbers in ascending order and store */

		if (num_tmp_relation > 1) {
			qsort(tmp_relations, (size_t)num_tmp_relation,
					sizeof(uint32), compare_uint32);
		}
		curr_relset->relation_list = (uint32 *) malloc(
					num_tmp_relation * sizeof(uint32));
		memcpy(curr_relset->relation_list, tmp_relations,
				num_tmp_relation * sizeof(uint32));

		/* store the list of large ideals; the first item in the
		   list points back to curr_relset */

		curr_relset->ideal_list = NULL;
		if (num_tmp_ideal) {
			list_t *ideal_list = (list_t *)malloc(
					(num_tmp_ideal + 1) * sizeof(list_t));
			ideal_list[0].payload.relset_num = num_relset;
			ideal_list[0].prev = NULL;
			ideal_list[0].next = NULL;
			for (j = 1; j <= num_tmp_ideal; j++) {
				ideal_list[j].payload.ideal = tmp_ideals[j-1];
			}
			curr_relset->ideal_list = ideal_list;
		}
		num_relset++;
	}

	logprintf(obj, "merged %u relations\n", num_merged);
	...
	for (i = 0; i < num_relset; i++) {
		list_t *list = relset_array[i].ideal_list;
		for (j = 1; j <= relset_array[i].num_large_ideals; j++) {
			uint32 ideal = list[j].payload.ideal;
			ideal_map[ideal].payload++;
		}
	}
	...
	for (i = 0; i < num_relset; i++) {
		...
		for (j = 1; j <= relset_array[i].num_large_ideals; j++) {
			uint32 ideal = list[j].payload.ideal;
			list[j].payload.ideal = ideal_map[ideal].payload;
		}
	}
