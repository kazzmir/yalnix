...
static void dump_cycles(msieve_obj *obj, merge_t *merge) {
	...
	fwrite(&num_relsets, (size_t)1, sizeof(uint32), cycle_fp);

	for (i = 0; i < num_relsets; i++) {
		relation_set_t *r = relset_array + i;
		uint32 num = r->num_relations;

		r->relation_list[num - 1] |= (uint32)(0x80000000);
		fwrite(r->relation_list, (size_t)num,
				sizeof(uint32), cycle_fp);
	}
	fclose(cycle_fp);
}
...
uint32 nfs_filter_relations(msieve_obj *obj, mp_t *n) {
	...
	filtmin = nfs_purge_duplicates(obj, &fb);
	...
	filter.filtmin_r = filtmin;
	filter.filtmin_a = filtmin;
	...
	/* give up if there is not enough excess to form a matrix
	   ...
	   construct a matrix). Also give up if we do not have at
	   least 70% excess and more sieving is an option */
	...
	while (1) {
		...
		filter.filtmin_r = filtmin_pass2;
		filter.filtmin_a = filtmin_pass2;
