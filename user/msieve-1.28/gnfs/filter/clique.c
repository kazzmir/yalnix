...
static void delete_relations(filter_t *filter,
	...
	filter->relation_array = (relation_ideal_t *)realloc(relation_array,
...
uint32 nfs_purge_cliques(msieve_obj *obj, 
	...
	num_clique = 0;
	clique_heap = (clique_t *)malloc(clique_heap_size * sizeof(clique_t));

	num_clique_relations_alloc = 500;
	clique_relations = (uint32 *)malloc(num_clique_relations_alloc *
					sizeof(uint32));

	num_clique_ideals_alloc = 500;
	clique_ideals = (uint32 *)malloc(num_clique_ideals_alloc *
					sizeof(uint32));
	...
	for (i = 0; i < num_ideals; i++) {
		...
		while (curr_clique_ideal < num_clique_ideals) {
			...
			while (offset != 0) {
				...
				if (num_clique_relations ==
					...
					clique_relations = (uint32 *)realloc(
						clique_relations,
						num_clique_relations_alloc *
						sizeof(uint32));
				}
		...
		next_clique->relation_array_word = 
				(uint32 *)malloc(num_clique_relations *
						sizeof(uint32));
	...
	num_delete = 0;
	num_ideals_delete = 0;
	delete_array = (uint32 *)malloc(num_delete_alloc * sizeof(uint32));
	...
	for (i = 0; i < num_clique; i++) {
		...
		for (j = 0; j < curr_clique->num_relations; j++) {
			if (num_delete == num_delete_alloc) {
				num_delete_alloc *= 2;
				delete_array = (uint32 *)realloc(delete_array,
							num_delete_alloc *
							sizeof(uint32));
			}
