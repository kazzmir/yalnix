...
static void matrix_weight_init(matrix_weight_t *m) {
	...
	m->weights = (uint32 *)calloc((size_t)m->num_weights_alloc,
					sizeof(uint32));
}
...
static void matrix_weight_add(matrix_weight_t *m, relation_set_t *r) {
	...
	if (w >= m->num_weights_alloc) {
		uint32 i;
		m->weights = (uint32 *)realloc(m->weights, 
					(size_t)(w + 100) *
					sizeof(uint32));
...
static void merge_two_relsets(relation_set_t *r1,
	...
	uint32 max_ideals = r1->num_large_ideals + r2->num_large_ideals - 1;
	...
	/* if r1 and r2 have only one large ideal, we assume
	   r_out is a cycle */

	if (r1->num_large_ideals == 1 &&
	    r2->num_large_ideals == 1) {
		r_out->num_large_ideals = 0;
		r_out->ideal_list = NULL;
		return;
	}

	/* get ready to merge the ideal lists of r1 and r2 */
...
static uint32 estimate_new_weight(relation_set_t *r1,
	...
	uint32 num_small;
	i = j = 0;
	while (i < r1->num_relations && j < r2->num_relations) {
		uint32 rel1 = r1->relation_list[i];
		uint32 rel2 = r2->relation_list[j];
		if (rel1 < rel2) {
			i++;
		}
		else if (rel1 > rel2) {
	...
	/* count the number of large ideals that would
	   cancel if r1 and r2 were merged */
	...
	return k + num_small + 
		(num_ideals1 - i + 1) + (num_ideals2 - j + 1);
}
...
static void merge_via_pivot(merge_aux_t *aux) {
	...
	/* separate the pivot relation set out */
	...
	free(pivot.relation_list);
	free(pivot.ideal_list);
}
...
static void do_merges_core(merge_aux_t *aux) {
	...
	if (aux->num_relsets == 1) {
		...
		free(aux->tmp_relsets[0].relation_list);
		free(aux->tmp_relsets[0].ideal_list);
...
/*--------------------------------------------------------------------*/
static void toggle_ideal_state(ideal_list_t *list, uint32 ideal, 
	...
	while (r != relset_head) {
		...
		if (active) {
			if (--curr_r->num_active_ideals == 0) {
				(*num_cycles)++;
				matrix_weight_add(mat_weight, curr_r);
			}
		}
		else {
			if (curr_r->num_active_ideals++ == 0) {
				(*num_cycles)--;
				matrix_weight_sub(mat_weight, curr_r);
			}
		}
		r = r->next;
	}
	...
	heap_node->payload.ideal_info.active ^= 1;
}
...
static uint32 store_next_relset_group(merge_aux_t *aux,
	...
	for (i = 0; i < aux->num_relsets - 1; i++) {
		...
		*r = aux->tmp_relsets[i];
		...
		/* if there are too many relation sets in r, 
		   delete it */

		if (r->num_relations > MAX_RELSET_SIZE) {
			free(r->relation_list);
			free(r->ideal_list);
			memset(r, 0, sizeof(relation_set_t));
			continue;
		}

		/* make the ideal list in r point back to r */

		if (r->ideal_list) {
			r->ideal_list[0].next = NULL;
			r->ideal_list[0].prev = NULL;
			r->ideal_list[0].payload.relset_num = 
						relset_num;
		}
...
void nfs_merge_full(msieve_obj *obj, merge_t *merge, uint32 min_cycles) {
	...
	for (i = num_cycles = 0; i < num_relsets; i++) {
		...
		free(r->ideal_list);
		r->ideal_list = NULL;
		if (r->relation_list && r->num_active_ideals == 0) {
	...
	merge->relset_array = relset_array = 
			(relation_set_t *)realloc(relset_array, num_cycles * 
