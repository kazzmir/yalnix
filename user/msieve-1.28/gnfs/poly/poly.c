...
int32 find_poly(msieve_obj *obj, mp_t *n) {
	...
	uint32 deadline;
	dd_precision_t precision;

	logprintf(obj, "commencing number field sieve polynomial selection\n");

	/* do sanity checking */

	if (obj->nfs_lower > 100 || obj->nfs_upper > 100) {
		printf("bounds must be between 1 and 100\n");
		return -1;
	}
	if (obj->nfs_lower != 0 && obj->nfs_upper != 0) {
		...
		logprintf(obj, "warning: piecewise poly selection ignored\n");
	}
...
#if 0
	find_poly_skew(obj, n, &config, deadline);	/* not ready yet */
#else
	...
	/* restore the floating point precision */
