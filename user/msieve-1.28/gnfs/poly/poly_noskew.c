...
static void save_poly(mp_t *n, poly_config_t *config,
...
void find_poly_noskew(msieve_obj *obj, mp_t *n,
	...
	noskew_data.cofactor_batch = (mp_t *)malloc(COFACTOR_BATCH_SIZE * 
						sizeof(mp_t));
	if ( ! noskew_data.cofactor_batch ){
	...
	for (i = min_large_factor; i < max_large_factor; i++) {
		...
		for (j = 0; j < COFACTOR_BATCH_SIZE; j++) {
			...
			for (c5 = d5; ; x++) {
				...
				save_poly(n, config, &tmp1, &high_coeff);
			}
		}
	}
}
...
static void save_poly(mp_t *n, poly_config_t *config,
	...
	memset(&rpoly, 0, sizeof(rpoly));
	memset(&apoly, 0, sizeof(apoly));
...
static void cofactor_sieve_init(mp_t *base, cofactor_sieve_t *c) {
	...
	c->sieve = (uint8 *)aligned_malloc(COFACTOR_SIEVE_SIZE, 16);
	if ( ! c->sieve ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
	}
...
static void init_cofactor_large(cofactor_sieve_t *c, 
	...
	sieve = (uint8 *)malloc((size_t)(sieve_size / 8 + 1));
	if ( ! sieve ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
	}
	...
	c->large_factors = (uint32 *)malloc(j * sizeof(uint32));
	if ( ! c->large_factors ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
	}
