...
typedef struct {
	...
	uint32 cutoff;		 /* fudge factor for trial factoring */
	uint32 scaled_cutoff;	 /* fudge factor for trial factoring */
...
static void init_one_fb(msieve_obj *obj, 
			...
			uint32 lp_cutoff_bits, char *string);
...
static uint32 find_large_primes(mp_t *res, 
			sieve_t *sieve_fb, uint32 *lp);

static void print_relation(msieve_obj *obj, int64 a, uint32 b, 
			uint32 *factors_r, uint32 num_factors_r, 
			uint32 *large_prime_r,
			uint32 *factors_a, uint32 num_factors_a, 
			uint32 *large_prime_a);
	
static uint32 fplog(uint32 k, double log_of_base);

static int32 fplog_eval_poly(int64 a, uint32 b, 
			mp_poly_t *f, double log_base);

static double get_log_base(mp_poly_t *poly, 
			int64 a0, int64 a1, uint32 b);

static uint32 read_last_line(msieve_obj *obj, mp_t *n);

static void write_last_line(msieve_obj *obj, mp_t *n, uint32 b);
...
uint32 do_line_sieving(msieve_obj *obj, sieve_param_t *params, mp_t *n,
	...
	init_one_fb(obj, &fb.rfb, &job.sieve_rfb, job.num_buckets, 
			params->rfb_lp_size, params->rfb_lp_cutoff_bits, 
			"RFB");
	init_one_fb(obj, &fb.afb, &job.sieve_afb, job.num_buckets, 
			params->afb_lp_size, params->afb_lp_cutoff_bits, 
			"AFB");
	...
	job.resieve_array = (resieve_t *)malloc(2 * MAX_RESIEVE_ENTRIES *
						sizeof(resieve_t));
	if ( ! job.resieve_array ){
	...
	for (i = 0; i <= job.max_b - job.min_b; i++) {
		...
		if ((relations_found >= next_print ||
		    i == job.max_b - job.min_b) &&
		    (obj->flags & (MSIEVE_FLAG_USE_LOGFILE |
	    		   	  MSIEVE_FLAG_LOG_TO_STDOUT))) {
			fprintf(stderr, "b = %u, found %u relations "
					"(need %u)\r", job.min_b + i, 
					relations_found, max_relations);
			fflush(stderr);
			next_print = relations_found + 50;
		}
...
static void init_one_fb(msieve_obj *obj, fb_side_t *fb, sieve_t *out_fb, 
	...
	out_fb->fb_entries = (fb_sieve_entry_t *)realloc(fb->entries,
						fb->num_entries * 
	...
	out_fb->sieve_block = (uint8 *)malloc(BLOCK_SIZE * sizeof(uint8));
	if ( ! out_fb->sieve_block ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
	}
	out_fb->bucket_list = (uint32 *)malloc(num_buckets * sizeof(uint32));
	if ( ! out_fb->bucket_list ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
	}
	out_fb->num_updates_alloc = 10000;
	out_fb->update_list = (packed_fb_t *)malloc(out_fb->num_updates_alloc * 
						sizeof(packed_fb_t));
	if ( ! out_fb->update_list ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
	}
	...
	out_fb->LP2_max.nwords = 2;
	out_fb->LP2_max.val[1] = 1 << (lp_cutoff_bits - 32);

	/* Determine the cutoff for log values. Sieve offsets whose
	   unaccounted log value is smaller than this will have trial
	   factoring attempted. */

	out_fb->cutoff = (uint32)(2.6 * log((double)largest_p) / M_LN2 + 0.5);
	
	/* If the large prime cutoff (plus a little) exceeds the trial
	   factoring cutoff, use that instead */

	high_bound = 4 + mp_bits(&out_fb->LP2_max);
	out_fb->cutoff = MAX(out_fb->cutoff, high_bound);
	...
	out_fb->fb_powers = (fb_power_t *)malloc(out_fb->powers_fb_alloc *
						sizeof(fb_power_t));
	if ( ! out_fb->fb_powers ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
	...
	logprintf(obj, "projective %s roots: %u\n", string, 
					out_fb->proj_fb_size);
	logprintf(obj, "%s trial factoring cutoff: %u bits\n", string, 
							out_fb->cutoff);
...
static void init_one_sieve(sieve_t *out_fb,
	...
	if (b_offset % LOG_UPDATE_RATE == 0) {
		...
		/* convert the trial factoring cutoff to this base */

		out_fb->scaled_cutoff = (uint32)((double)out_fb->cutoff * 
...
static void fill_one_block(sieve_t *sieve_fb, 
	...
	while (fb_offset != 0) {
		...
		if (i >= num_updates_alloc) {
			...
			update_list = (packed_fb_t *)realloc(update_list, 
							num_updates_alloc * 
							sizeof(packed_fb_t));
...
static uint32 do_factoring(sieve_job_t *job, 
	...
	int32 cutoff_r, cutoff_a, cutoffL, cutoffR;
	int64 a, gcda; 
	...
	a = block_start;
	cutoffL = fplog_eval_poly(a, b, rfb->poly, rfb->log_base);
	cutoffR = fplog_eval_poly(a + real_block_length, b, 
				rfb->poly, rfb->log_base);
	cutoff_r = (cutoffL + cutoffR)/2 - rfb->scaled_cutoff;
	if (cutoff_r < 0)
		cutoff_r = 0;

	cutoffL = fplog_eval_poly(a, b, afb->poly, afb->log_base);

	for (i = 0; i < BLOCK_SIZE; i += num_scanned) {

		/* calculate the cutoff for the next
		   num_scanned algebraic sieve values */

		a += A_SAMPLE_RATE;
		cutoffR = fplog_eval_poly(a, b, afb->poly, afb->log_base);
		cutoff_a = (cutoffL + cutoffR)/2 - afb->scaled_cutoff;
		if (cutoff_a < 0)
			cutoff_a = 0;
		cutoffL = cutoffR;
...
static uint32 do_one_factoring(sieve_job_t *job, resieve_t *sieve_value,
	...
	num_factors_a = do_one_tf(afb, &a_res.num, 
				sieve_value, factors_a, offset, b);
	if (num_factors_a == 0)
		return 0;

	eval_poly(&r_res, a, b, rfb->poly);
	num_factors_r = do_one_tf(rfb, &r_res.num, 
				sieve_value + 1, factors_r, offset, b);
	if (num_factors_r == 0)
		return 0;

	/* find the large primes, or quit if the large
	   primes are too big. Unlike the trial factoring,
	   here we start with the *smallest* cofactor, since
	   if it fails then it will at least fail faster */

	if (mp_cmp(&r_res.num, &a_res.num) > 0) {
		if (find_large_primes(&a_res.num, afb, lp_a) != 1)
			return 0;
		if (find_large_primes(&r_res.num, rfb, lp_r) != 1)
			return 0;
	}
	else {
		if (find_large_primes(&r_res.num, rfb, lp_r) != 1)
...
static uint32 do_one_tf(sieve_t *sieve_fb, 
			...
			uint32 *factors, uint32 offset, uint32 b) {
	...
	/* continue only if the cofactor remaining after 
	   trial division is small, or composite and the 
	   right size for large primes. A base-2 pseudoprime 
...
static uint32 find_large_primes(mp_t *res, 
	...
	/* Use heavy-duty methods to break up an input
	   cofactor into its component large primes */

	lp[0] = lp[1] = 1;
	if (res->nwords == 1) {
		/* cofactor is smaller than the large prime bound B
		   and so is autmatically prime */

		lp[0] = res->val[0];
	}
	else {
		/* Use SQUFOF on the cofactor; quit if either
		   of the factors exceed the allowable size of 
		   large primes */
		...
		mp_divrem_1(res, f, res);
		if (res->val[0] > sieve_fb->LP1_max || 
		    f > sieve_fb->LP1_max)
			return 0;
...
static int32 fplog_eval_poly(int64 a, uint32 b, 
	...
	return (uint32)(mp_bits(&norm.num) * M_LN2 / log_base);
}
