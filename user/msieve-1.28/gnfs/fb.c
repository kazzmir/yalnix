...
void create_factor_base(msieve_obj *obj, 
	...
	rfb->num_alloc = 10000;
	rfb->entries = (fb_entry_t *)malloc(rfb->num_alloc*sizeof(fb_entry_t));
	if (! rfb->entries ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
	}
	afb->num_alloc = 10000;
	afb->entries = (fb_entry_t *)malloc(afb->num_alloc*sizeof(fb_entry_t));
	if ( ! afb->entries ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
	}
	...
	while(1) {
		...
		if (p <= max_prime_r) {
			...
			if (num_found_r + num_roots >= rfb->num_alloc) {
				rfb->num_alloc *= 2;
				rfb->entries = (fb_entry_t *)realloc(
							rfb->entries,
							rfb->num_alloc * 
							sizeof(fb_entry_t));
			}
		...
		if (p <= max_prime_a) {
			...
			if (num_found_a + num_roots >= afb->num_alloc) {
				afb->num_alloc *= 2;
				afb->entries = (fb_entry_t *)realloc(
							afb->entries,
							afb->num_alloc * 
							sizeof(fb_entry_t));
			}
	...
	rfb->entries = (fb_entry_t *)realloc(rfb->entries,
				num_found_r * sizeof(fb_entry_t));
	afb->entries = (fb_entry_t *)realloc(afb->entries,
...
int32 read_factor_base(msieve_obj *obj, mp_t *n,
	...
	while (!feof(fp)) {
		...
		else if (strstr(buf, "SRTFBITS"))
			params->rfb_lp_cutoff_bits = value;
		else if (strstr(buf, "SATFBITS"))
			params->afb_lp_cutoff_bits = value;
	...
	fb->rfb.entries = (fb_entry_t *)malloc((rfb_size + MAX_POLY_DEGREE) * 
					sizeof(fb_entry_t));
	if ( ! fb->rfb.entries ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
	...
	fb->afb.entries = (fb_entry_t *)malloc((afb_size + MAX_POLY_DEGREE) * 
					sizeof(fb_entry_t));
	if ( ! fb->afb.entries ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
...
void write_factor_base(msieve_obj *obj, mp_t *n,
	...
	fprintf(fp, "SRTFBITS %u\n", params->rfb_lp_cutoff_bits);
	fprintf(fp, "SATFBITS %u\n", params->afb_lp_cutoff_bits);
