...
static int stack_push(eval_stack_t *stack, 
	...
	curr_entry = (char *)malloc((size_t)(symbol_len + 1));
	if (curr_entry == NULL)
...
int32 evaluate_expression(char *expr, mp_t *res) {
	...
	if (*tmp == 0) {
		...
		else if (tmp - expr > 150) {
			/* there's only one reason you're trying to
			   factor a general number this big */
			fprintf(stderr, "\nwarning: no credit card number / "
					"secret email / X-BOX game is worth "
					"all the work you are about to do\n");
		}
