...
void save_relation(sieve_conf_t *conf, uint32 sieve_offset,
	...
	qs_print_to_savefile(conf->obj, buf);
...
static void enumerate_cycle(msieve_obj *obj, 
	...
	c->cycle.list = (uint32 *)malloc(c->cycle.num_relations * 
					sizeof(uint32));
	if ( ! c->cycle.list ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
...
void qs_filter_relations(sieve_conf_t *conf) {
	...
	relation_list = (relation_t *)malloc(10000 * sizeof(relation_t));
	if ( ! relation_list ){
		TtyPrintf( 0, "no free memory\n" );
		exit( -1 );
	}
		...
		case 'R':
			...
			if (start != NULL) {
				...
				if (i == curr_rel) {
					...
					relation_list = (relation_t *)realloc(
							relation_list,
							curr_rel *
							sizeof(relation_t));
				}
	...
	num_relations = purge_singletons(obj, relation_list, num_relations,
					table, hashtable);
	relation_list = (relation_t *)realloc(relation_list, num_relations * 
							sizeof(relation_t));
	...
	cycle_list = (la_col_t *)malloc(num_cycles * sizeof(la_col_t));
	if ( ! cycle_list ){
		logprintf( obj, "no free memory\n" );
		exit( -1 );
	}
